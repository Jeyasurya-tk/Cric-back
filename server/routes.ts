      ...
      if (matchStatus === 'completed') {
        ...
        if (updatedMatch.tournamentId) {
          ...
          if (tournament && (tournament.tournamentType === 'League + Knockout')) {
            ...
            const remainingLeagueMatches = await (MatchModel as any).countDocuments({ 
            ...
            if (remainingLeagueMatches === 0) {
              const hasPlayoffs = await (MatchModel as any).exists({ 
                tournamentId: tournament._id, 
            ...
            if (updatedMatch.stage === 'semifinal') {
              const remainingSemis = await (MatchModel as any).countDocuments({
                tournamentId: tournament._id,
                stage: 'semifinal',
                status: { $ne: 'completed' }
              });
              ...
              if (remainingSemis === 0) {
                const semiFinals = await (MatchModel as any).find({
                  tournamentId: tournament._id,
                  stage: 'semifinal'
                });
